...
public class AugmentedTriadSymbol extends TriadSymbol {

	public AugmentedTriadSymbol(Pitch root, Inversion inversion) {
		super(root, inversion);
	}

	public AugmentedTriadSymbol(Pitch root, Inversion inversion, Set<ChordSymbolAlteration> modifications,
			Optional<ChordConstraint> constraint) {
		super(root, inversion, modifications, constraint);
	}

	public AugmentedTriadSymbol(Pitch root, Pitch bass, Set<ChordSymbolAlteration> modifications, Optional<ChordConstraint> constraint) {
	...
	protected Set<Pitch> chordPitchesWithoutModifications() {
		...
		pitches.add(
				new Interval(new Note(ROOT, -1), 5, new IntervalQuality(IntervalQuality.Type.AUGMENTED), Pitch.Role.FIFTH).HIGH_NOTE.PITCH);
		return pitches;
	}
